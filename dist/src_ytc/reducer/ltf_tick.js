      ...
      if (length >= 1) {
        ...
        if (action.payload.potentialSwing) {
          ...
          var _action$payload2 = action.payload,
              high = _action$payload2.high,
              low = _action$payload2.low,
              potentialTrend = _action$payload2.potentialTrend,
              bewareOfPotentialSignalCandle = _action$payload2.bewareOfPotentialSignalCandle;
          var _action$payload$poten = action.payload.potentialSwing,
              potentialDirection = _action$payload$poten.direction,
              candle = _action$payload$poten.candle;
          var potentialHigh = candle.high,
              potentialLow = candle.low;
          ...
          if (potentialDirection == "down") {
            ...
            if (action.payload.lowPivotsCombo) {
              var nearbyPivotArray = action.payload.lowPivotsCombo.map(function (e) {
                return Object.values(e)[0];
              });
              var breachedArray = nearbyPivotArray.filter(function (h) {
                return h < action.payload.high && h > action.payload.low;
              });
            ...
            if (low < potentialLow && high < potentialHigh && bewareOfPotentialSignalCandle && potentialTrend !== "potential-downswing") {
              var _bewareOfPotentialSig = bewareOfPotentialSignalCandle.candle,
                  bewareLow = _bewareOfPotentialSig.low,
                  bewareHigh = _bewareOfPotentialSig.high;
          ...
          if (potentialDirection == "up") {
            ...
            if (action.payload.upPivotsCombo) {
              var _nearbyPivotArray = action.payload.upPivotsCombo.map(function (e) {
                return Object.values(e)[0];
              });
              var breachedArray = _nearbyPivotArray.filter(function (h) {
                return h < action.payload.high && h > action.payload.low;
              });
              if (breachedArray.length >= 1) {
                if (action.payload.close < breachedArray[0]) {
            ...
            if (high > potentialHigh && low > potentialLow && bewareOfPotentialSignalCandle && potentialTrend !== "potential-uptrend") {
              var _bewareOfPotentialSig2 = bewareOfPotentialSignalCandle.candle,
                  _bewareHigh = _bewareOfPotentialSig2.high,
                  _bewareLow = _bewareOfPotentialSig2.low;
        ...
        if (parseFloat(state[state.length - 1].high) >= parseFloat(action.payload.high) && parseFloat(state[state.length - 1].low) <= parseFloat(action.payload.low)) {
          ...
          if (state[state.length - 1].potentialSwing && state[state.length - 2].potentialSwing) {
            ...
            if (currentPotentialDirection == previousPotentialDirection && currentPotentialDirection == "down" && currentTicktype == "green") {
              ...
              if (action.payload.potentialSwing) {
                var potentialDir = action.payload.potentialSwing.direction;
            ...
            if (currentPotentialDirection == previousPotentialDirection && currentPotentialDirection == "up" && currentTicktype == "red") {
              ...
              if (action.payload.potentialSwing) {
                ...
                var _potentialDir = action.payload.potentialSwing.direction;

                if (_potentialDir !== "down") {
          ...
          if (action.payload.tickType == "red") {
            ...
            if (action.payload.potentialSwing) {
              ...
              var _potentialDir2 = action.payload.potentialSwing.direction;

              if (_potentialDir2 !== "down") {
                //////// console('Looking for potential downtrend at..' + action.payload.date);
                action.payload.potentialSwing = { direction: 'down', candle: action.payload, prevCandle: prevSwingCandle };
                if (bewareMode) {
          ...
          if (action.payload.tickType == "green") {
            ...
            if (action.payload.potentialSwing) {
              var _potentialDir3 = action.payload.potentialSwing.direction;

              if (_potentialDir3 !== "up") {
                action.payload.potentialSwing = { direction: 'up', candle: action.payload, prevCandle: prevSwingCandle };
                if (bewareMode) {
                  action.payload.bewareOfPotentialSignalCandle = state[state.length - 1].potentialSwing;
                } else {
        ...
        if (parseFloat(state[state.length - 1].high) >= parseFloat(action.payload.high) && parseFloat(state[state.length - 1].low) <= parseFloat(action.payload.low)) {
          ...
          if (state[state.length - 1].potentialSwing && state[state.length - 2].potentialSwing) {
            ...
            if (currentPotentialDirection == previousPotentialDirection && currentPotentialDirection == "down" && currentTicktype == "green") {
              ...
              var comboState = [].concat(_toConsumableArray(state));
              ...
              if (!state[state.length - 1].aggregatePotentialSwings) {
                last.aggregatePotentialSwings = [_defineProperty({}, candleDate, potentialStrength)];
              } else {
                last.aggregatePotentialSwings = [].concat(_toConsumableArray(state[state.length - 1].aggregatePotentialSwings), [_defineProperty({}, action.payload.date, potentialStrength)]);
              }

              if (!state[state.length - 1].potentialSwingExtremeCombo) {
                last.potentialSwingExtremeCombo = [_defineProperty({}, candleDate, potentialSwingExtreme1)];
              } else {
                last.potentialSwingExtremeCombo = [].concat(_toConsumableArray(state[state.length - 1].potentialSwingExtremeCombo), [_defineProperty({}, action.payload.date, potentialSwingExtreme1)]);
              }

              var newCombo = [].concat(comboState, [last]);
              return newCombo;
            }
          ...
          if (state[state.length - 1].specialOverlappingCondition) {
            ...
            if (!state[state.length - 1].aggregatePotentialSwings) {
              last.aggregatePotentialSwings = [_defineProperty({}, candleDate1, potentialStrength1)];
            } else {
              last.aggregatePotentialSwings = [].concat(_toConsumableArray(state[state.length - 1].aggregatePotentialSwings), [_defineProperty({}, action.payload.date, potentialStrength1)]);
            }

            if (!state[state.length - 1].potentialSwingExtremeCombo) {
              last.potentialSwingExtremeCombo = [_defineProperty({}, candleDate1, potentialSwingExtreme1)];
            } else {
              last.potentialSwingExtremeCombo = [].concat(_toConsumableArray(state[state.length - 1].potentialSwingExtremeCombo), [_defineProperty({}, action.payload.date, potentialSwingExtreme1)]);
            }
          ...
          if (!state[state.length - 1].aggregatePotentialSwings) {
            last.aggregatePotentialSwings = [_defineProperty({}, dailyCandleDate, potentialCandleTrend)];
          } else {
            last.aggregatePotentialSwings = [].concat(_toConsumableArray(state[state.length - 1].aggregatePotentialSwings), [_defineProperty({}, action.payload.date, potentialCandleTrend)]);
          }

          if (!state[state.length - 1].potentialSwingExtremeCombo) {
            last.potentialSwingExtremeCombo = [_defineProperty({}, dailyCandleDate, potentialSwingExtreme1)];
          } else {
            last.potentialSwingExtremeCombo = [].concat(_toConsumableArray(state[state.length - 1].potentialSwingExtremeCombo), [_defineProperty({}, action.payload.date, potentialSwingExtreme1)]);
          }
            ...
            if (direction == prevone && beforeprevone != prevone && direction != "same") {
              ...
              if (direction == "down") {
                ...
                if ((action.payload.downPivotNotFormed != true || action.payload.downPivotNotFormed != undefined) && weakPivot == false) {
                  ...
                  if (parseFloat(state[state.length - 1].high) > parseFloat(state[state.length - 2].high)) {
                    ...
                    action.payload.upPivots = _defineProperty({}, state[state.length - 2].date, parseFloat(state[state.length - 1].high));
                  } else {
                    //debugger;
                    ...
                    action.payload.upPivots = _defineProperty({}, state[state.length - 2].date, parseFloat(state[state.length - 2].high));
                    //action.payload.potentialSwing = null;
                  }
                  ...
                  if (!state[state.length - 1].upPivotsCombo) {
                    ...
                    action.payload.upPivotsCombo = [].concat(_toConsumableArray(state[state.length - 1].upPivotsCombo), [action.payload.upPivots]);
                  }
                  ...
                  if (!state[state.length - 1].aggregatePotentialSwings) {
                    action.payload.aggregatePotentialSwings = [_defineProperty({}, action.payload.date, action.payload.potentialTrend)];
                  } else {
                    action.payload.aggregatePotentialSwings = [].concat(_toConsumableArray(state[state.length - 1].aggregatePotentialSwings), [_defineProperty({}, action.payload.date, action.payload.potentialTrend)]);
                  }

                  if (!state[state.length - 1].potentialSwingExtremeCombo) {
                    action.payload.potentialSwingExtremeCombo = [_defineProperty({}, action.payload.date, action.payload.potentialSwingExtreme)];
                  } else {
                    action.payload.potentialSwingExtremeCombo = [].concat(_toConsumableArray(state[state.length - 1].potentialSwingExtremeCombo), [_defineProperty({}, action.payload.date, action.payload.potentialSwingExtreme)]);
                  }
                ...
                if (action.payload.upPivotNotFormed != true || action.payload.upPivotNotFormed != undefined) {
                  ...
                  if (state[state.length - 1].low <= state[state.length - 2].low) {
                    ...
                    action.payload.lowPivots = _defineProperty({}, state[state.length - 2].date, parseFloat(state[state.length - 1].low));
                  } else {
                    action.payload.lwpCandle = state[state.length - 1].close;
                    action.payload.lwpCandleTime = state[state.length - 1].date;
                    action.payload.pivot = state[state.length - 2].low;
                    action.payload.lowPivots = _defineProperty({}, state[state.length - 2].date, parseFloat(state[state.length - 2].low));
                  }

                  if (!state[state.length - 1].lowPivotsCombo) {
                    action.payload.lowPivotsCombo = [action.payload.lowPivots];
                  } else {
                    action.payload.lowPivotsCombo = [].concat(_toConsumableArray(state[state.length - 1].lowPivotsCombo), [action.payload.lowPivots]);
                  }
              ...
              if (direction == "up") {
                if (parseFloat(state[state.length - 2].high) >= parseFloat(action.payload.high)) {
                  ...
                  if (!state[state.length - 1].aggregatePotentialSwings) {
                    last.aggregatePotentialSwings = [_defineProperty({}, action.payload.date, action.payload.potentialTrend)];
                  } else {
                    last.aggregatePotentialSwings = [].concat(_toConsumableArray(state[state.length - 1].aggregatePotentialSwings), [_defineProperty({}, action.payload.date, action.payload.potentialTrend)]);
                  }

                  if (!state[state.length - 1].potentialSwingExtremeCombo) {
                    last.potentialSwingExtremeCombo = [_defineProperty({}, action.payload.date, action.payload.potentialSwingExtreme)];
                  } else {
                    last.potentialSwingExtremeCombo = [].concat(_toConsumableArray(state[state.length - 1].potentialSwingExtremeCombo), [_defineProperty({}, action.payload.date, action.payload.potentialSwingExtreme)]);
                  }
                ...
                if (parseFloat(state[state.length - 2].low) <= parseFloat(action.payload.low)) {
                  ...
                  var obj = [].concat(_toConsumableArray(state));
