  ...
  switch (action.type) {
    ...
    case 'ADD_DATA_SCOPE_HOUR':
      ...
      if (length >= 1) {
        ...
        var concerned_high_pivot = 0;
        var concerned_low_pivot = 0;

        var all_nearby_high_pivots = action.payload.trimmedHighPivots;
        var all_nearby_low_pivots = action.payload.trimmedLowPivots;
        ...
        if (all_nearby_high_pivots && all_nearby_high_pivots.length >= 1 && action.payload.up_pivots_tested && action.payload.up_pivots_tested.length >= 1) {
          all_nearby_high_pivots = all_nearby_high_pivots.filter(function (x) {
            return !action.payload.up_pivots_tested.includes(x);
          });
        }
        ...
        if (all_nearby_low_pivots && all_nearby_low_pivots.length >= 1 && action.payload.down_pivots_tested && action.payload.down_pivots_tested.length >= 1) {
          all_nearby_low_pivots = all_nearby_low_pivots.filter(function (x) {
            return !action.payload.down_pivots_tested.includes(x);
          });
        }
        ...
        if (concerned_high_pivot && action.payload.high >= concerned_high_pivot && concerned_high_pivot >= action.payload.low && (action.payload.potentialTrend == "do-not-enter" || action.payload.potentialTrend == "potential-uptrend" || action.payload.potentialTrend == null)) {
          ...
          if (action.payload.up_pivots_tested == null) {
            ...
            action.payload.up_pivots_tested = [].concat(_toConsumableArray(action.payload.up_pivots_tested), [concerned_high_pivot]);
          }
        ...
        if (concerned_low_pivot && action.payload.high >= concerned_low_pivot && concerned_low_pivot >= action.payload.low && (action.payload.potentialTrend == "do-not-enter" || action.payload.potentialTrend == "potential-downswing" || action.payload.potentialTrend == null)) {
          ...
          if (action.payload.down_pivots_tested == null) {
            ...
            action.payload.down_pivots_tested = [].concat(_toConsumableArray(action.payload.down_pivots_tested), [concerned_low_pivot]);
          }
        ...
        if (action.payload.noLong || action.payload.noShort) {
          var _action$payload5 = action.payload,
              currentOpen = _action$payload5.open,
              currentHigh = _action$payload5.high,
              currentLow = _action$payload5.low,
              currentDate = _action$payload5.date,
              currentClose = _action$payload5.close,
              currentTickType = _action$payload5.tickType;
          var _state = state[state.length - 1],
              prevOpen = _state.open,
              prevHigh = _state.high,
              prevLow = _state.low,
              prevDate = _state.date,
              prevClose = _state.close,
              prevTickType = _state.tickType;
          // Fir
