  ...
  if (combinedSendTradeArr.length > 0) {
    ...
    combinedSendTradeArr.map(function (value, index) {
      ...
      if (type === "delta" && deltaIndex !== -1) {
        ...
        var diffqty = value.quantity - openOrderBook[deltaIndex].quantity;
        if (diffqty !== 0) {
    ...
    if (spreadValue > 2 * tickSize) {
      ...
      if (asksArray.length > 1 && asksQuantity.length > 1) {
        ...
        for (var i = 0; i < asksArray.length; i++) {
          ...
          var _pricedata = parseFloat(asksArray[i].toFixed(4));
          ...
          var data = { 'quantity': value.Qty, 'cm': cmdata, 'price': _pricedata, 'instrument': instrumentName };
      ...
      if (asksArray.length === 1 && asksQuantity.length === 1) {
        ...
        var _pricedata2 = parseFloat(asksArray[0].toFixed(4));

        var data = { 'quantity': value.Qty, 'cm': asksCM[0], 'price': _pricedata2, 'instrument': instrumentName };
      ...
      if (bidsArray.length > 1 && bidsQuantity.length > 1) {
        ...
        for (var i = 0; i < bidsArray.length; i++) {
          ...
          var _pricedata3 = parseFloat(bidsArray[i].toFixed(4));
          var _quantdata = parseFloat(bidsQuantity[i].toFixed(4));
          ...
          var _cmdata = parseFloat(cm.toFixed(4));
          var data = _defineProperty({ 'quantity': value.Qty, 'cm': _cmdata, 'price': _pricedata3, 'instrument': instrumentName }, 'cm', cm);
      ...
      if (asksArray.length > 1 && asksQuantity.length > 1) {
        for (var i = 0; i < asksArray.length; i++) {
          ...
          var _pricedata4 = parseFloat(asksArray[i].toFixed(4));
          var _quantdata2 = parseFloat(asksQuantity[i].toFixed(4));
          ...
          var _cmdata2 = parseFloat(cm.toFixed(4));
          var data = { 'quantity': value.Qty, 'cm': _cmdata2, 'price': _pricedata4, 'instrument': instrumentName };
          optiondata[globalindex].CleanedSellOrderBook.push(data);
        }
      }
      ...
      if (bidsArray.length > 1 && bidsQuantity.length > 1) {
        ...
        for (var i = 0; i < bidsArray.length; i++) {
          ...
          var _pricedata5 = parseFloat(bidsArray[i].toFixed(4));
          var _quantdata3 = parseFloat(bidsQuantity[i].toFixed(4));
          var _cmdata3 = parseFloat(cm.toFixed(4));

          var data = _defineProperty({ 'quantity': value.Qty, 'cm': _cmdata3, 'price': _pricedata5, 'instrument': instrumentName }, 'cm', cm);
          optiondata[globalindex].CleanedBuyOrderBook.push(data);
        }
      }
    } else {
...
async function processOptionData(array) {
  ...
  try {
    var _loop = function _loop() {
      ...
      tradedData = _.without(_.map(newdata.result, function (o) {
        if (o.instrument == item.instrumentName) return o;
      }), undefined);
      quant = 0;

      tradedData.map(function (value, index) {
        quant = parseInt(value.quantity) + quant;
      });
      item.volume = quant;
  ...
  try {
    ...
    try {
      for (var _iterator2 = array[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        ...
        var data = await getSummaryData(_item);
  ...
  try {
    ...
    try {
      for (var _iterator3 = array[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        ...
        var data = await getOrdebookCallData(_item2);
        collatedData.push(data);
      }
    } catch (err) {
    ...
    if (collatedData.length === array.length) {
      ...
      collatedData = collatedData.filter(function (n) {
        return n;
      });

      collatedData = _.sortBy(collatedData, 'Spread').reverse();
      collatedData.map(function (value, index) {
...
function getDeltaHedeger(openPosition) {
  ...
  var btcIndex = _.findIndex(openPosition, function (valueD) {
    return valueD.instrument === "BTC-PERPETUAL";
  });
